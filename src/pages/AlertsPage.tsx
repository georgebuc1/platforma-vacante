...
import { Bell, Mail, MapPin, Wallet, Globe, Compass, Calendar, Clock, CheckCircle } from 'lucide-react';
...
export default function AlertsPage() {
  ...
  const validate = () => {
    ...
    if (!consent) errs.consent = 'Trebuie să fii de acord cu primirea notificărilor.';
    setErrors(errs);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEmail(''); setCountry(''); setTripType('orice'); setMonth('oricand');
      setDuration('orice'); setFrequency('immediate'); setConsent(false);
    } catch {
    ...
    <div className="container-page py-10">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900">Nu rata următoarea ofertă bună.</h1>
          <p className="mt-2 text-slate-500">Spune-ne ce cauți și te putem anunța când apare o ofertă potrivită.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="card p-6 sm:p-8 space-y-5">
          ...
          <div>
            <label className="label-field flex items-center gap-1.5">
              <MapPin className="h-4 w-4 text-brand-500" /> De unde pleci? <span className="text-error-500">*</span>
            </label>
          ...
          <div>
            <label className="label-field flex items-center gap-1.5">
              <Wallet className="h-4 w-4 text-brand-500" /> Buget maxim (RON) <span className="text-error-500">*</span>
            </label>
            <input type="number" min="0" step="50" value={maxBudget} onChange={(e) => setMaxBudget(e.target.value)} className="input-field" placeholder="2500" />
            {errors.maxBudget && <p className="mt-1 text-xs text-error-600">{errors.maxBudget}</p>}
          </div>
          <div>
            <label className="label-field flex items-center gap-1.5">
              <Globe className="h-4 w-4 text-brand-500" /> Țara preferată <span className="text-slate-400 font-normal">(opțional)</span>
            </label>
            <input type="text" value={country} onChange={(e) => setCountry(e.target.value)} className="input-field" placeholder="Ex: Grecia" />
          </div>
          <div>
            <label className="label-field flex items-center gap-1.5">
          ...
          <div>
            <label className="label-field flex items-center gap-1.5">
              <Calendar className="h-4 w-4 text-brand-500" /> Perioada
            </label>
          ...
          <div>
            <label className="label-field flex items-center gap-1.5">
              <Clock className="h-4 w-4 text-brand-500" /> Durata
            </label>
          ...
          <div>
            <label className="label-field">Frecvența notificărilor</label>
            <div className="grid grid-cols-3 gap-2">
              {FREQUENCIES.map((f) => (
                <button key={f.value} type="button" onClick={() => setFrequency(f.value)}
                  className={`rounded-xl border px-4 py-3 text-sm font-semibold transition-colors ${frequency === f.value ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                  {f.label}
                </button>
              ))}
            </div>
          ...
          <div>
            <label className="flex items-start gap-3 cursor-pointer">
              <input type="checkbox" checked={consent} onChange={(e) => setConsent(e.target.checked)} className="mt-0.5 h-5 w-5 rounded border-slate-300 text-brand-600 focus:ring-brand-500" />
              <span className="text-sm text-slate-600">
                Sunt de acord să primesc notificări despre ofertele care corespund criteriilor mele. <span className="text-error-500">*</span>
              </span>
            </label>
            {errors.consent && <p className="mt-1 text-xs text-error-600">{errors.consent}</p>}
          </div>
          <button type="submit" disabled={submitting} className="btn-primary w-full text-base py-4">
            <Bell className="h-5 w-5" />
            {submitting ? 'Se salvează...' : 'CREEAZĂ ALERTA'}
          </button>
          <div className="flex items-center gap-2 rounded-xl bg-slate-50 p-3 text-xs text-slate-500">
            <CheckCircle className="h-4 w-4 text-success-500" />
            <span>Alerta ta va fi salvată. Nu vei primi email-uri reale în această versiune.</span>
          </div>
        </form>
      </div>
    </div>
